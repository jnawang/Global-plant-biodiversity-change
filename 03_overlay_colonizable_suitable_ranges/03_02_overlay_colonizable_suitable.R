...
# 1. you may want to adjust shift.rate.lat.universal and shift.rate.ele.universal
# 2. adjust the SDM names and their range out directory
# 3. set up the grid distance on earth chunk directory
# 4. set the species id or species name to compute
#########################
library(terra)
library(tidyverse)
library(dismo)
...
######attention: remember to change the location where grid distance files are stored L333##########
...
#  name <- 'Rhagodia preissii'          # 'Cornus florida'    # 'Eriophorum angustifolium'  'Oreopanax flaccidus'
  ...
  ############################################################Create folders############################################
  folders <- c(migrations, 'area_shrink')
  for (iSDM in 1:nSDMs) {
  ...
  #-----------------------------------------------About to calculate reachable grids------------------------------------
  ...
  if (nrow(dt.idc.tbd.global.max) > 0) {
    ...
    for (ichunk in 1:length(chunk_grid_files)) {
      ...
      dt.distance0 <- read_fst(file.path('..', 'grid_distance_earth', 'chunks', paste0(chunk_grid_files[ichunk], '.fts')), as.data.table = TRUE)
      ...
      # add one column dif_lat, here need to consider the differences of calculating lat difference between south and north hemispheres.
  ...
  for (iSDM in 1:nSDMs) {
    ...
    for (iscenario in 1:nscenarios) {
      ...
      for (ishift.rate in 1:4) {
        ...
        writeRaster(r.future.real, file.path(SDM.path.output[iSDM], migrations[ishift.rate], scenarios[iscenario], paste0(name, '.tif')), overwrite=TRUE) 
      }
    }
...
# id = 53348    # testing species id

# # Skip this species if its calculation is already done!
# if (!file.exists(file.path(SDM.path.output[nSDMs], 'area_shrink', paste0(species[id], '.csv')))) {
#   get.realized.map(species[id])
# }
