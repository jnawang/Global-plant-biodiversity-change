...
setwd('/home/jnawang/SDM_process/novel_community/map2bin/')

read.filerange <- function(from, to) {
    data <- list()
    for (filenum in from:to) {
        read.file <- file(paste(filenum, "present.dat", sep = ""), "rb")
...
caculate.file <- function(ifile = 33) {
    bio1 <- terra::values(rast("/home/jnawang/SDM072023/preds/bios_pres/bio1.tif"))
    ...
    cell.max <- landcells[ifile * ncell.file + 1] + nneighbor + nneighbor * ncol
    ...
    for (i.cel in 1:length(cel_indices)) {
        ...
        cells.region <- vector()
        for (i in -5:5) {
            cells.region <- c(cells.region, (focal.cell - 5):(focal.cell + 5) + ncol * i)
        }
        # print(cells.region)

        valid_cells <- cells.region[!sapply(cells.region, function(idx) is.null(data[[idx]]))]
        if (length(valid_cells) < threshold) {
            result <- rbind(result, data.frame(
                cell = focal.cell, validnum = length(valid_cells),
                JTU = NA, JNE = NA, JAC = NA
            ))
        } else {
            ...
            if (length(valid_cells) == threshold) {
                ...
                valid_cells.beta <- beta.multi(valid_cells.m, index.family = "jaccard")
            } else {
                mat <- sapply(1:100, function(x) {
                    ...
                    unlist(beta.multi(valid_cells.m, index.family = "jaccard"))
                })
                mat <- t(mat)
            ...
            result <- rbind(result, data.frame(
                cell = focal.cell, validnum = length(valid_cells),
                JTU = valid_cells.beta$beta.JTU, JNE = valid_cells.beta$beta.JNE, JAC = valid_cells.beta$beta.JAC
            ))
            print(paste(i.cel, focal.cell, length(valid_cells), valid_cells.beta$beta.JAC))
        }
        # if (i.cel > 100) {
        #     break
        # }
    }
    write.csv(result, paste("/home/jnawang/SDM_process/richness/beta/beta-", ifile, ".csv", sep = ""), row.names = FALSE)
}
