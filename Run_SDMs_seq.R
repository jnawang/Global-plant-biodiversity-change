rm(list=ls())
gc()

# Run SDMs 

# This runs SDMs species by species (sequencial)

# force the cluster to identify the correct path to libraries
#.libPaths("/home/jnawang/R/x86_64-pc-linux-gnu-library")
#libs <- .libPaths()

#if(!'dismo' %in% installed.packages()[,"Package"]){
#  install.packages('dismo', repos='https://rspatial.r-universe.dev')
#}

########################
# Setup
list.of.packages <- c("raster","rgdal","rgeos","terra",
                      "SDMtune","dismo",
                      "tidyverse",
                      "RSQLite","DBI","odbc",
                      "foreach","doParallel","pbapply","doFuture","future","doRNG")

new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]

if(length(new.packages)) install.packages(new.packages, repos='http://cran.us.r-project.org')

sapply(list.of.packages, require, character.only = TRUE)

########################
# Source code
source("MadMax.R")

########################
# Args
command_args <- commandArgs(trailingOnly = TRUE)
spset <- as.numeric(command_args[1])
#spset <- 28475

# 17305 50808 38040 40446 38997 40266 28497 40224 17278 28475

########################
# Load occurrences

## species to model
# The RDS is just the name of these species
sptogo <- readRDS("plants_list.RDS")[spset]
# sptogo <- readRDS("sptorun.RDS")[spset]

# get occurrences for species in chunk i
drv <- dbDriver("SQLite") 
my_db_file <- "plant_occur.sqlite"
db <- dbConnect(drv, my_db_file)

# the sqlite only stores location information of each species
mydata <- dbGetQuery(
  db, 
  paste0(
    "SELECT * ",
    "FROM occur ",
    "WHERE sps in (", paste(shQuote(sptogo, type = "sh"),collapse = ', '), ")")
)

DBI::dbDisconnect(db)

# project coords to mollweide
# convert matrix to a SpatialPoints object
pts.moll <- sp::SpatialPoints(mydata[,1:2], 
                              proj4string = CRS("+proj=longlat"))
# convert to a mollweide
pts.moll <- sp::spTransform(pts.moll, CRSobj = "+proj=moll")
pts.moll <- data.frame(pts.moll)
########################
# load predictors

# soil and aridity
asoil <- list.files("preds/soil", pattern = ".tif$", full.names = T)
asoil <- terra::rast(asoil)

# present
bios_pres <- list.files("preds/bios_pres", pattern = ".tif$", full.names = T)
bios_pres <- terra::rast(bios_pres)

bios_pres <- c(bios_pres,asoil)

# projections variables to the future
# we choose ensemble average of 16 scenarios
gcms  <- c("ESM")
ssps  <- c(126, 245, 370, 585)
times <- c("2021-2040", "2041-2060", "2061-2080", "2081-2100")
futvars <- expand.grid(gcms, ssps, times)
futvars$go <- paste(futvars[,1],futvars[,2],futvars[,3],sep="_")

bios_fut <- list()
for(i in 1:length(futvars$go)){
  bios_fut. <- list.files("preds/bios_fut", 
                          pattern = paste0(futvars$go[i],".tif$"), 
                          full.names = T)
  bios_fut. <- terra::rast(bios_fut.)
  # fix names
  mypattern <- paste0("_",futvars$go[i])
  mypattern <- gsub("-",".",mypattern)
  names(bios_fut.) <- gsub(mypattern,"",names(bios_fut.))
  bios_fut. <- c(bios_fut.,asoil)
  
  bios_fut[[i]] <- bios_fut.
}
names(bios_fut) <- futvars$go
rm(asoil)



########################
# Run SDM

cat("\nRunning model for", unique(mydata$sps),"\n")

#### Using doFuture

m <-try({
  
  MadMax(spname = sptogo,
         occ = pts.moll,
         fit.vars = bios_pres,
         proj.vars = bios_fut, # list with predictors for future scenarios
         proj.names = futvars$go, # vector names for proj.vars in the same order as proj.vars
         output_dir = "Results",
         check = F)
  
},silent = T)

if(class(m) == "try-error"){
  error_dir <- paste0("Error")
  if(!dir.exists(error_dir)){
    dir.create(error_dir, recursive = TRUE)
  }
  write.csv(data.frame(sps = unique(mydata$sps),
                       error = m[1]),
            paste0(error_dir,"/",spset,".csv"))
  
  print(m)
}



########################
# RUN THIS?
YN <- "no"
if(YN == "yes"){
  # Find species that did not run
  # Maxent output
  spmax <- list.files("C:/Redist/Plants/Results/Maxnet", pattern = ".RDS")
  spmax <- gsub("_mod.RDS","",spmax)
  spmax <- gsub("_"," ",spmax)
  # thr
  spthr <- list.files("C:/Redist/Plants/Results/th", pattern = "thr.csv")
  spthr <- gsub("_thr.csv","",spthr)
  spthr <- gsub("_"," ",spthr)
  # stats
  spstats <- list.files("C:/Redist/Plants/Results/stats", pattern = "stats.csv")
  spstats <- gsub("_stats.csv","",spstats)
  spstats <- gsub("_"," ",spstats)
  # LogOutFut
  spLogOutFut <- list.files("C:/Redist/Plants/Results/LogOutFut", 
                            pattern = ".tif")
  spLogOutFut <- gsub(".tif","",spLogOutFut)
  spLogOutFut <- gsub("_"," ",spLogOutFut)
  # LogOutPres
  spLogOutPres <- list.files("C:/Redist/Plants/Results/LogOutPres", 
                             pattern = ".tif")
  spLogOutPres <- gsub(".tif","",spLogOutPres)
  spLogOutPres <- gsub("_"," ",spLogOutPres)
  # RangeFut
  spRangeFut <- list.files("C:/Redist/Plants/Results/RangeFut", 
                           pattern = ".tif")
  spRangeFut <- gsub(".tif","",spRangeFut)
  spRangeFut <- gsub("_"," ",spRangeFut)
  # RangePres
  spRangePres <- list.files("C:/Redist/Plants/Results/RangePres", 
                            pattern = ".tif")
  spRangePres <- gsub(".tif","",spRangePres)
  spRangePres <- gsub("_"," ",spRangePres)
  
  # Combine all
  sprun <- unique(c(spmax,spthr,spstats,spLogOutFut,spLogOutPres,spRangeFut,spRangePres))
  # get species absent from sptorun
  allsps <- readRDS("plants_list.RDS")
  
  sptorun <- allsps[which(!allsps %in% sprun)]
  saveRDS(sptorun, "sptorun.RDS")
}