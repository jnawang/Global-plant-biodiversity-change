...
data_plants <- data %>% filter(Kingdom == 'Plantae' & Ecosystem == 'Terrestrial')
...
barrier <- read.csv('pre_post_processing/bioshift_HFI_mountain.csv')        # Need to change A66_P1; 
...
plants_modeled   <- read.csv('pre_post_processing/plant_taxonomy_nloc.csv')
...
#######
#######
# trait_level model next; + (1|Quality);  + (1|Sampling)
...
data_env <- read.csv('pre_post_processing/env_data_predict_bioshift.csv')
...
data_env$Species <- gsub('.', ' ', data_env$Species, fixed = TRUE)    # 'fixed' prevent the use of reg-exp
...
# data_env_lat$AreaF    <- as.factor(4)
# cutoff threshold: 21673.87, 151136.0, 409778.0, 3090540.0
...
boot_lat_species <- lme4::bootMer(mod_lat_species, mySumm_lat_species, nsim=10, use.u=TRUE, type="parametric", verbose = TRUE) # use.u whether random effects are simulated/bootstrapped? should I use.u = TRUE? type = "parametric", "semiparametric"
mySumm_lat_trait <- function(.) {
  predict(., newdata=data_env_lat_trait, re.form=NULL)
}
boot_lat_trait <- lme4::bootMer(mod_lat_trait, mySumm_lat_trait, nsim=10, use.u=TRUE, type="parametric", verbose = TRUE) # use.u whether random 
...
data_env_ele$Sampling <- as.factor("MULT")    #  as.factor("MULT")
data_env_ele$StartF   <- as.factor(2)
data_env_ele$Quality  <- as.factor("RESURVEYED")
#
#
...
boot_ele_species <- lme4::bootMer(mod_ele_species, mySumm_ele_species, nsim=10, use.u=TRUE, type="parametric", verbose = TRUE) # use.u whether random effects are simulated/bootstrapped? should I use.u = TRUE? type = "parametric", "semiparametric"
mySumm_ele_trait <- function(.) {
  predict(., newdata=data_env_ele_trait, re.form=NULL)
}
boot_ele_trait <- lme4::bootMer(mod_ele_trait, mySumm_ele_trait, nsim=10, use.u=TRUE, type="parametric", verbose = TRUE) # use.u whether random 
...
write.csv(ShiftR_Final, 'pre_post_processing/Final_prediction_shift_rate.csv')
