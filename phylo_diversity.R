...
read.filerange <- function(from, to) {
  ...
  for (filenum in from:to) {
    read.file <- file(paste(filenum, "present.dat", sep = ""), "rb")      # Realized_ESM_370_2081-2100.dat
...
#  ifile <- 177
  ...
  tree   <- readRDS("/home/jnawang/SDM_process/richness/traits/plants_phy.RDS")
  # change tree label
  tree$tip.label <- gsub("_"," ", tree$tip.label)
  # read species list  
  # sps_id <- read.csv('C:/Users/jnawang/Box/Biodiversity/pre_post_processing/traits_phylo/model_sps_id.csv')
  sps_id <- read.csv('/home/jnawang/SDM_process/richness/traits/model_sps_id.csv')
  #
...
#  write.csv(output, paste("C:/Users/jnawang/Downloads/pd-", ifile, "present.csv", sep = ""), row.names = FALSE)  # Realized_ESM_370_2081-2100.dat
  write.csv(output, paste("/home/jnawang/SDM_process/richness/phylogenetic/pd-", ifile, "present.csv", sep = ""), row.names = FALSE)
}
