...
##effect size of random effects (ranef(mod_lat_trait)); we cannot adjust this. 
...
##AreaF: 0.09849527, 0.24359295, -0.14221958, -0.19986865
###################################################################Fit elev models#########################################################
###########################################################################################################################################
...
data_env_lat$AreaF    <- as.factor(1)       # M: 1; F: 2; L: 4;
#
...
data_env_ele$StartF   <- as.factor(3)              # M: 3; L: 2; F: 4;
data_env_ele$Quality  <- as.factor("MODELED")      # M: MODELED; L: LOW; F: RESURVEYED; but I use RESURVEYED, because modelled is too slow. 
...
# do the prediction for species level and traits level separately, and then merge.
# Still need to do: mean values and different credible values!
data_env_ele_species$EleShiftR <- predict(mod_ele_species, newdata = data_env_ele_species, re.form=~((1|StartF) + (1|Quality) + (1|Species)))
data_env_ele_trait$EleShiftR   <- predict(mod_ele_trait,   newdata = data_env_ele_trait,   re.form=~((1|StartF) + (1|Quality)))

# use a new way of doing prediction
mySumm_ele_species <- function(.) {
  predict(., newdata=data_env_ele_species, re.form=~((1|StartF) + (1|Quality) + (1|Species)))    # do not use Sampling for the median prediction, as mult and two have big differences. 
}
...
mySumm_ele_trait <- function(.) {
  predict(., newdata=data_env_ele_trait, re.form=~((1|StartF) + (1|Quality)))                    # do not use Sampling for the median prediction, as mult and two have big differences. 
}
...
write.csv(ShiftR_Final, 'pre_post_processing/02_predict_shift_rate/02_03_perform_shift_rate_prediction/Final_prediction_shift_rate_median_2025.csv')
