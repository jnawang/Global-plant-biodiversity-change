# model selection for predicting bioshift rate
...
f="ShiftR ~ BaselineT2 + g_Mount + g_varT + g_hetE + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Species)"
...
## this is just a repeated process!
if(isSingular(mod1)==T){ #in case of the selected model (with REML=T) is singular, a new random model structure selection is conducted substituing (1|Family/Genus) by (1|Genus)
  f="ShiftR ~ BaselineT2 + g_Mount + g_varT + g_hetE + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Family/Genus)"
  tx=testSingAl(x,f,data,n=1:length(x)) #take some time
...
if(isSingular(mod1)==T){ #in case of the selected model (with REML=T) is singular, a new random model structure selection is conducted without any phylogenetic effect
  ...
  if(isSingular(mod1)==T){#in case of no model selected considering our criteria (convergence and non signularity), a more simple random model structure is tested: (1|Family/Genus)
    f1="ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_hetE + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Species)"                              # why two response variables? Junna
    mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
    if(isSingular(mod1)==T){ #in case of singularity, a more simple random model structure is tested: (1|Genus)
      f1="ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_hetE + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Family/Genus)"
      mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
      if(isSingular(mod1)==T){ #in case of singularity, a more simple random model structure is tested: (1|Source)
        f1="ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_hetE + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Source)"
        mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
        if(isSingular(mod1)==T){ #in case of singularity, we finally fit a simple linear model
          ...
          mod1=lm(ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_hetE + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI, data)
        }
      }
    }
  }
...
if(isSingular(mod1)==F){
  ...
  write.table(as.data.frame(ms1),"elevation_selAIC.csv",sep=",",dec=".",row=F)
  ms1E=ms1
...
f="ShiftR ~ BaselineT2 + g_Mount + g_varT + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Species)"
tx=testSingAl(x,f,data,n=1:length(x)) #take some time
...
if(isSingular(mod1)==T){ #in case of the selected model (with REML=T) is singular, a new random model structure selection is conducted substituing (1|Family/Genus) by (1|Genus)
  f="ShiftR ~ BaselineT2 + g_Mount + g_varT + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Family/Genus)"
  tx=testSingAl(x,f,data,n=1:length(x)) #take some time
...
if(isSingular(mod1)==T){ #in case of the selected model (with REML=T) is singular, a new random model structure selection is conducted without any phylogenetic effect
  ...
  # I think these codes are unnecessary!
  if(isSingular(mod1)==T){#in case of no model selected considering our criteria (convergence and non signularity), a more simple random model structure is tested: (1|Family/Genus)
    f1="ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Species)"
    mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
    if(isSingular(mod1)==T){ #in case of singularity, a more simple random model structure is tested: (1|Genus)
      f1="ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI + (1|Family/Genus)"
      mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
      if(isSingular(mod1)==T){ #in case of singularity, a more simple random model structure is tested: (1|Source)
        f1="ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI +(1|Source)"
        mod1=lmer(as.formula(f1),data,REML=TRUE,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
        if(isSingular(mod1)==T){ #in case of singularity, we finally fit a simple linear model
          print("Impossible to fit the linear mixed-effect model due to singularity issue")
          mod1=lm(ShiftR ~ ShiftR ~ BaselineT2 + g_Mount + g_varT + g_height + g_seedmass + g_velT * g_basT + g_velT * g_HFI, data)
        }
      }
    }
  }
...
if(isSingular(mod1)==F){
  mod2=lmer(formula(mod1),data,REML=F,na.action="na.fail",control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
  ms1=dredge(mod2) #take some time: ms1 is the table reporting results of the model selection through AICc
  write.table(as.data.frame(ms1),"latT_selAIC.csv",sep=",",dec=".",row=F)
  ms1Lm=ms1
